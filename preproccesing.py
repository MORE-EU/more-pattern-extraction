...
def change_granularity(df,granularity='30s',size=10**7,chunk=True): 
    ...
    Args:
        ...
        size: The size/chunks we want to divide our /DataFrame according to the global index of the set.
        The Default price is 10 million.       .
    ...
    if chunk==True: #Getting rid of NaN occurances.
        df=chunk_interpolate(df,size=size, method="linear", axis=0,limit_direction="both", limit=1)
...
def chunk_interpolate(df,size=10**6, method="linear", axis=0,limit_direction="both", limit=1):
